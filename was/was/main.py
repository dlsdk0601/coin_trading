...
def app():
    ...
    try:
        # BUY / SELL / HOLD 결정
        ret = upbit.trading(ticker=CoinType.KRW_BTC.label)

        # 현재 보유 현금 (소수 1번째 반올림)
        current_cash = int(round(upbit.get_balance_cash(), 0))

        # 현재 코인 가격
        current_price = upbit.get_current_price(ticker=CoinType.KRW_BTC.label)

        # 현재 코인 보유 수량 (소수 3번째 반올림)
        current_balance = round(upbit.get_balance_coin(ticker=CoinType.KRW_BTC.label), 2)

        # 평균 매수 가격
        buy_avg_price = upbit.get_buy_average(ticker=CoinType.KRW_BTC.label)

        # 평가 손익 / 수익률
        margin, margins = get_margin(current_price, buy_avg_price, current_balance)

        log.log('TR', f'  잔액 : {current_cash:,d}원')  # ',d'는 천 단위 구분자 추가
        log.log('TR', f'보유량 : {current_balance:,.2f} {CoinType.KRW_BTC.label}')
